...
- [x] Run focused Hive tests and full `@franken/brain` test/lint/typecheck/build plus `git diff --check` (496/496 tests; concurrent migration stress 10/10).
- [x] Independently audit the diff for blocking data-loss/concurrency/privacy regressions; fixed the checkpoint race and additive-Hive restart finding, then reran all gates.
