...
- [x] Run the 54 focused runtime tests, 13 chat-server tests, focused CLI governor-wiring test, package lint/typecheck/build, and root lint/typecheck/build successfully after the exact-base rebase. Root `npm run test` still reports seven unrelated established full-suite failures (three load-sensitive timeouts, network health environment state, two CLI mock-order failures, and one CR-progress timing failure); all changed-path focused tests pass independently.
...
- [ ] Rebase the remediation onto #3821's final exact-current-head clean commit (currently `4bfd23c168ecc62f3be86a295a53bc9881e55207`, green CI but four unresolved Codex threads) and rerun focused/package/root gates before publication.
