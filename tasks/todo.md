...
- 2026-07-23: Fixed a live-reported bug in the already-merged chat provider self-awareness feature (ADR-040, PR #3505): the runtime-status transparency note was appended directly onto the raw user-turn prompt during CLI session continuation, making it indistinguishable from injected content — Claude's own anti-injection training correctly flagged and refused to trust it, undermining the whole feature. Fixed by adding a `systemPromptAddendum` option threaded through `LlmCompletionOptions` → `AdapterLlmClient` → `CliLlmAdapter.execute()` → `ProviderOpts`, delivered via Claude's `--append-system-prompt` (gated by new `ICliProvider.supportsSystemPromptAddendum?()`, true only for `ClaudeProvider`; Codex/Gemini keep the prior append-to-prompt behavior, which was already observed to work for them). Verified live before implementing: `--append-system-prompt` content is trusted even when it contradicts the model's default self-report, and survives `--continue`/`--resume`. Verified end-to-end against the real compiled build afterward. New branch `fix/chat-provider-note-system-channel` off latest `main` (PR #3505 had already merged). Full suite green: `packages/franken-orchestrator` 283 files / 4435 tests, `packages/franken-types` 11 files / 130 tests; typecheck and build clean in both. See ADR-040's third "Update" section for full detail.
