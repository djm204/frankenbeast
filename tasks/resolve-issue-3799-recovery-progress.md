...
## Exact-head CI scan-cap performance recovery (`7a97feee4`)

- [x] Reproduced the two existing 65,536-character scan-cap regressions locally without changing their timeouts, fixture sizes, or behavioral assertions: the combined focused run passed but consumed 3.56s of test time (4.04s Vitest duration).
- [x] Profiled the unchanged redaction path and identified the URL-userinfo discovery regex as the actual hotspot: its scheme prefix can restart at every word boundary in long hyphenated non-URL text and rescan the remaining prefix quadratically before the later bounded PowerShell/POSIX logic runs.
- [x] Strict deterministic RED: added URL-userinfo scheme-character work metrics to both existing cap tests before changing candidate discovery. The focused run failed 2/2 while preserving the expected redacted outputs: PowerShell modeled 304,164,082 comparisons against a 152,000 linear budget; POSIX modeled 235,664,061 against a 147,200 linear budget. Test execution remained 3.45s.
- [x] Minimal GREEN: replaced only the backtracking URL-userinfo candidate regex with a forward boundary/scheme/username scanner and retained the existing password boundary, truncated-tail, active-expression, authority, and literal-redaction behavior. The identical two-test command passed 2/2 with linear counts of 76,094 and 73,690 characters; test execution fell to 62ms (508ms Vitest duration, 0.96s wall clock).
- [x] URL-userinfo compatibility passed 8/8. The requested multiline/PowerShell/POSIX/physical-line sibling selection passed 17/17.
...
## Current-head nineteenth-review seven-P1 recovery (`57512ea1ce43aa5d3228c1e5baf7234234024bd9`)

- [x] Finding 1 RED: `npm test --workspace @franken/governor -- --run tests/unit/channels/cli-channel.test.ts -t "redacts ordinary braced PowerShell variables while preserving executable substitutions"` failed 1/1 (262 skipped), exposing both ordinary braced values.
- [x] Finding 1 GREEN: the identical command passed 1/1 (262 skipped) after admitting an unscoped identifier in the existing braced PowerShell alternative; the same bounded quoted scanners preserve `$(Write-Output harmless-braced)` and `Restart-Computer`.
- [x] Finding 2 RED: the focused `redacts complete TOML triple-quoted sensitive strings` command failed 1/1 (263 skipped), leaving both multiline suffixes visible after partially redacting the first quote.
- [x] Finding 2 GREEN: the identical command passed 1/1 (263 skipped) after a bounded triple-delimiter prepass consumed the complete basic/literal values and the ordinary quote passes explicitly ignored triple openers.
- [x] Finding 3 RED: the focused `preserves pure spaced legacy backtick substitutions in sensitive values` command failed 1/1 (264 skipped), receiving `PASSWORD=[REDACTED] harmless legacy\`` and hiding the active command prefix.
- [x] Finding 3 GREEN: the identical command passed 1/1 (264 skipped) after applying the existing complete-expression guard to a parser-confirmed leading backtick expression as well as `$()`.
- [x] Finding 4 RED: the focused `redacts complete HCL heredoc sensitive values` command failed 1/1 (265 skipped), exposing both ordinary and indented heredoc bodies.
- [x] Finding 4 GREEN: the identical command passed 1/1 (265 skipped) after a line-anchored, 65,536-character-capped matching-terminator scan consumed each body while retaining `<<EOT`/`<<-TOKEN`, terminators, and `command = "reboot"`.
- [x] Finding 5 RED: the focused `classifies quoted terminal segments in dotted TOML keys` command failed 1/1 (266 skipped), exposing both quoted-terminal values.
- [x] Finding 5 GREEN: the identical command passed 1/1 (266 skipped) after a line-anchored dotted-key parser decoded and classified only the terminal quoted segment while retaining key spelling and the safe sibling.
- [x] Finding 6 RED: the focused `consumes complete PowerShell collection literals in sensitive assignments` command failed 1/1 (267 skipped), truncating both collections at internal whitespace and exposing their suffixes.
- [x] Finding 6 GREEN: the identical command passed 1/1 (267 skipped) after a balanced, quote-aware, 65,536-character-capped `@(...)`/`@{...}` scan retained collection syntax, property names, semicolons, both complete `$()` expressions, and `Restart-Computer` while redacting literal fragments.
- [x] Finding 7 RED: the focused `consumes complete TOML arrays and inline tables for sensitive keys` command failed 1/1 (268 skipped), partially redacting the opening tokens and exposing collection suffixes.
- [x] Finding 7 GREEN: the identical command passed 1/1 (268 skipped) after a bounded balanced collection prepass plus a complete-collection guard for later token passes; TOML structure, property names, both `$()` expressions, and the safe sibling remain visible.
- [x] Exact seven-test aggregate passed 7/7 (262 skipped) using the requested CLI-channel command with the seven regression names joined by `|`.
- [x] Relevant sibling selection `PowerShell|backtick|substitution|collection|TOML|HCL|heredoc|dotted configuration` passed 63/63 (206 skipped).
- [x] Complete CLI-channel verification passed 269/269; full Governor verification passed 588/588 across 25 files.
- [x] Governor typecheck and build passed. Lint passed with zero errors and exactly the three established unrelated warnings in `src/policy.ts` and `tests/integration/full-approval-flow.test.ts`. Final `git diff --check` passed.
- [x] Final status/name/stat/scoped-hunk inspection confirmed exactly the authorized production redactor, focused CLI-channel test, and this recovery progress document are modified. New structured scans are line-anchored and/or capped at 65,536 characters, balanced scans are single-pass, synthetic fixtures contain no real credentials, and assertions retain collection/heredoc syntax, operators, `$()`/backtick substitutions, and following commands. The worktree remains intentionally uncommitted; no GitHub or prohibited repository mutation occurred.

## Nineteenth-round independent uncommitted-review follow-ups

- [x] Finding 1 RED: `npm test --workspace @franken/governor -- --run tests/unit/channels/cli-channel.test.ts -t "ignores escaped quote runs when closing TOML multiline basic strings"` failed 1/1 (269 skipped), closing at the escaped quote run and misclassifying the suffix/safe line. GREEN: the identical command passed 1/1 (269 skipped) after a bounded parity-aware multiline-basic terminator scan; the literal-string path retains its bounded literal delimiter lookup.
- [x] Finding 2 RED: `npm test --workspace @franken/governor -- --run tests/unit/channels/cli-channel.test.ts -t "decodes TOML Unicode escapes in quoted dotted sensitive keys"` failed 1/1 (270 skipped), exposing the `pass\u0077ord` value. GREEN: the identical command passed 1/1 (270 skipped) after strict bounded TOML short/`\u`/`\U` decoding with Unicode scalar validation before terminal-key classification.
- [x] Finding 3 RED: `npm test --workspace @franken/governor -- --run tests/unit/channels/cli-channel.test.ts -t "consumes TOML collections assigned through quoted dotted sensitive keys"` failed 1/1 (271 skipped), exposing both collection values. GREEN: the identical command passed 1/1 (271 skipped) after sharing terminal-key classification and extending the bounded collection assignment parser to quoted/bare dotted segments.
- [x] Finding 4 RED: `npm test --workspace @franken/governor -- --run tests/unit/channels/cli-channel.test.ts -t "preserves TOML collection syntax while redacting credential strings"` failed 1/1 (272 skipped), corrupting escaped-string punctuation and `foo.bar` while replacing `true`/`42`. GREEN: the identical command passed 1/1 (272 skipped) with a TOML-aware single-pass fragment redactor that preserves delimiters, keys, punctuation, primitives, `$(printf harmless-toml)`, and the safe following mapping while hiding credential strings.
- [x] Aggregate/siblings: the exact four-test selection passed 4/4 (269 skipped); the unchanged prior seven-test aggregate passed 7/7 (266 skipped); `-t "TOML|PowerShell|HCL|heredoc|substitution|collection"` passed 66/66 (207 skipped).
- [x] Full verification: complete CLI-channel passed 273/273; full Governor passed 592/592 across 25 files; package typecheck and build passed; lint had zero errors and exactly the same three established unrelated warnings in `src/policy.ts` and `tests/integration/full-approval-flow.test.ts`; `git diff --check` passed.
- [x] Final inspection confirms exact existing three-file scope, 65,536-character/depth-bounded collection discovery, bounded linear TOML fragment/escape scans, synthetic-only fixtures, hidden credential literals, and retained substitutions, delimiters, dotted keys, primitives, punctuation, and following safe context. Work remains intentionally uncommitted with no prohibited repository or GitHub action.

## Nineteenth-round TOML triple-quoted key-path P1 follow-up

- [x] Strict RED: `npm test --workspace @franken/governor -- --run tests/unit/channels/cli-channel.test.ts -t "redacts triple-quoted values for quoted and dotted TOML sensitive keys"` failed 1/1 (273 skipped). The quoted bare key and both bare-/quoted-terminal dotted sensitive key paths leaked or were only partially redacted, while the synthetic safe dotted sibling remained visible.
- [x] Minimal GREEN: the triple-quoted assignment scanner now accepts the same bounded TOML quoted/bare segment grammar and uses the shared `isSensitiveTomlKeyPath` terminal classifier already used by scalar and collection handlers. Ordinary mixed-word and quoted-dotted handlers skip triple openers so the bounded triple scanner consumes the complete value. The identical focused command passed 1/1 (273 skipped).
- [x] Verification: prior TOML follow-ups passed 4/4 (270 skipped); the prior seven GitHub regressions passed 7/7 (267 skipped); `TOML|PowerShell|HCL|heredoc|substitution|collection` siblings passed 67/67 (207 skipped); complete CLI-channel passed 274/274; full Governor passed 593/593 across 25 files. Package typecheck and build passed; lint had zero errors and exactly the same three established unrelated warnings in `src/policy.ts` and `tests/integration/full-approval-flow.test.ts`; `git diff --check` passed.
- [x] Final inspection confirmed exactly the existing production redactor, focused CLI-channel test, and this progress document are modified. TOML key-path matching is line-bounded with disjoint quoted/bare segment alternatives; multiline value discovery remains capped at 65,536 characters and scans forward once (with only a backward parity count over the immediately adjacent backslash run), preserving bounded linear behavior. The worktree remains intentionally uncommitted and no prohibited repository, dependency, secret, or GitHub operation was performed.

## Nineteenth-round independent collection/comment and bounded-literal follow-up

- [x] Finding 1 RED→GREEN: isolated `ignores TOML line-comment syntax while balancing multiline sensitive collections` failed 1/1 (274 skipped), exposing both post-comment credential suffixes; a TOML-aware 65,536-character/64-depth scanner now skips `#` comments only outside basic/literal strings through CR/LF, and the identical test passed 1/1 (274 skipped) while retaining comment text and collection syntax.
- [x] Finding 2 RED→GREEN: isolated `ignores PowerShell comment closers while balancing both collection forms` failed 1/1 (275 skipped), exposing the later credential in all four line/block-comment × `@()`/`@{}` cases; bounded comment state plus the existing quote/escape/depth state passed the identical test 1/1 (275 skipped), preserving all four `$()` expressions and following commands.
- [x] Finding 3 RED→GREEN: isolated `bounds TOML multiline literal closer lookup to the configured scan budget` failed 1/1 (276 skipped), modeling 65,557 examined characters against the 65,536 budget; slicing once to `scanEnd` before literal-delimiter lookup made the identical deterministic metric pass 1/1 (276 skipped), with no unbounded rescan.
- [x] Exact aggregate and nineteenth-round evidence: the three new regressions passed 3/3 (274 skipped); the prior seven findings, four independent follow-ups, and triple-quoted key-path follow-up passed 12/12 (265 skipped).
- [x] Relevant sibling and full verification: `-t "TOML|PowerShell|HCL|heredoc|substitution|comment|bound|collection"` passed 88/88 (189 skipped); complete CLI-channel passed 277/277; full Governor passed 596/596 across 25 files. Package typecheck and build passed; lint had zero errors and exactly the same three established unrelated warnings in `src/policy.ts` and `tests/integration/full-approval-flow.test.ts`; `git diff --check` passed.
- [x] Final inspection confirms the exact existing production redactor, CLI-channel test, and progress document are the only modified files. TOML and PowerShell collection discovery each use one forward scan capped at 65,536 characters and 64 nested openers; comment state changes only outside strings, TOML comments end at CR/LF, PowerShell block comments end at `#>`, and literal-string lookup operates on one bounded slice. Work remains intentionally uncommitted; no commit, push, comment, review/retrigger, thread resolution, merge, branch, secret, dependency, or other remote/repository mutation was performed.

## Nineteenth-round structured TOML collection P2 follow-up

- [x] Add focused quoted dotted TOML-key and JSON colon-key coverage, including escaped quoted key tokens; the existing focused case is GREEN and preserves exact key spelling/punctuation while redacting only values.
- [x] Add focused multiline basic/literal TOML strings inside arrays/inline tables, including basic escapes, policy-permitted substitutions, comments, and following safe context; strict RED processed triple quotes as separate ordinary strings. The narrow GREEN recognizes complete triple-delimited tokens inside the already bounded collection slice, preserves delimiters, applies the existing active-substitution policy only to multiline basic strings, and wholly redacts multiline literal contents.
- [x] Exact focused command `npm test --workspace @franken/governor -- --run tests/unit/channels/cli-channel.test.ts -t "preserves quoted dotted TOML and JSON colon keys inside sensitive collections|redacts complete TOML multiline strings inside sensitive collections"` passed 2/2 (277 skipped).
- [x] The unchanged prior seven-GitHub-finding selection passed 7/7 (272 skipped), and `git diff --check` passed. Status and stat inspection retained exactly the existing three modified files; no test-file edit was needed for this slice.
- [x] Independent verification passed the combined nine-test current-head plus structured-collection selection (9/9), the relevant `TOML|PowerShell|HCL|heredoc|JSON|substitution|comment|bound|collection` sibling selection (96/96), the complete CLI-channel file (279/279), and all Governor tests (598/598 across 25 files).
- [x] Governor package typecheck and build passed. Lint passed with zero errors and exactly the same three established unrelated warnings in `src/policy.ts` and `tests/integration/full-approval-flow.test.ts`; `git diff --check` passed.
- [x] Final pre-publication inspection confirmed exact three-file scope: the production redactor, focused CLI-channel tests, and this progress document. TOML/PowerShell/HCL scans retain the established 65,536-character and depth bounds, credential literals remain hidden, and syntax, substitutions, operators, comments, and following safe command context remain asserted.

## Current-head twentieth-review eight-P1 recovery (`0dd19c87b33b92360128c0fd30bff2edac7d4ea4`)

- [x] Findings 1–2 preserved and validated the existing partial uncommitted implementation/tests: RFC 3986 userinfo accepts every sub-delimiter plus complete percent-encoded bytes before the credential colon, and established sensitive-YAML shell context retains every nonempty no-argument command while pre-context plaintext remains redacted. The exact aggregate below passed both cases; the relevant sibling selection also required updating the older post-context no-argument expectation to the newly required visible behavior.
- [x] Findings 3–8 strict RED: `npm test --workspace @franken/governor -- --run tests/unit/channels/cli-channel.test.ts -t "GNU Make conditional|attached MySQL|aws configure set|pwd aliases|provider tokens|Telegram bot tokens"` failed all six new focused regressions (6 failed, 281 skipped), respectively exposing the synthetic Make value, both MySQL attached passwords, the AWS positional value, both `pwd` alias values, the `sk_` token, and both Telegram path tokens.
- [x] Finding 3 GREEN: a line-anchored GNU Make `?=` pass classifies only the assignment key and preserves safe assignments and following recipe context.
- [x] Finding 4 GREEN: command-bounded MySQL-family handling covers attached `-pPASSWORD` only for `mysql`, `mysqldump`, `mysqladmin`, and `mysqlcheck`; unrelated `psql -p5432` and `custom -pvisible` remain exact, while balanced executable substitutions remain visible between redacted literal fragments.
- [x] Finding 5 GREEN: command-bounded `aws configure set <key> <value>` handling classifies the positional key, uses the existing bounded shell-value scanner, retains executable substitutions, and preserves operators/following commands and nonsensitive settings.
- [x] Finding 6 GREEN: add the exact `pwd` vocabulary alias, covering `DB_PWD` and connection-string `Pwd` through the existing boundary-aware assignment passes.
- [x] Finding 7 GREEN: the explicit provider-token format accepts both `sk-` and `sk_` prefixes with the unchanged 20-character minimum and bounded token alphabet.
- [x] Finding 8 GREEN: Telegram API `/botTOKEN` and `/webhooks/telegram/TOKEN` redaction uses the three patterns aligned with `packages/franken-orchestrator/src/security/telegram-redaction.ts`, including colon and percent-encoded-colon token separators and bounded path terminators.
- [x] Exact eight-test aggregate passed 8/8 (279 skipped) using a test-name regex anchored at the end for the eight named regressions. Relevant `URL userinfo|YAML shell context|assignment|password|token|webhook|CLI|command` siblings passed 114/114 (173 skipped).
- [x] Complete CLI-channel verification passed 287/287; all `@franken/governor` tests passed 606/606 across 25 files. Package typecheck and build passed. Lint passed with zero errors and exactly the same three established unrelated warnings in `src/policy.ts` and `tests/integration/full-approval-flow.test.ts`; `git diff --check` passed.
- [x] Final scope/security inspection confirms only the authorized production redactor, focused CLI-channel test, and this existing progress document are modified. All fixtures are synthetic; URL percent bytes are complete, AWS/MySQL scans are command/value bounded, shell substitutions/operators remain visible, and no commit, push, branch/worktree/PR/issue/card mutation, GitHub reply/resolution, review trigger, or merge was performed.
