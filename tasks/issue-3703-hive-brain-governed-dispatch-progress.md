...
- Independent current-head audit found the daemon adapter still used the restartable generic stop route. RED regressions now require a dedicated authenticated reject route and deny restart with or without a linked run; daemon rejection terminally records `rejected`, and the focused suites plus lint, typecheck, build, and diff-check pass.
- Follow-up current-head audit closed the remaining terminality seams: run-level start/restart and CLI-backed controls reject denied linked agents, in-flight executor starts stop without overwriting `rejected`, dispatch races recheck terminal state, and idempotent REST/WebSocket rejection retries clean stale Beast context before returning.
- A separate exact-head blocking audit then found two repository transaction time-of-check/time-of-use windows. Capacity reservation, run-status synchronization, and start-failure bookkeeping now re-read terminal status inside their write transactions and never overwrite a concurrently committed `rejected`; focused race regressions pass.
