...
- JSON sanitizers must recurse when the top-level parsed value is itself a string, recognize sensitive header-entry tuples such as `["Authorization", value]`, proxy-authorization variants, and sibling-labelled `{name|key, value}` pairs regardless of property order, and preserve duplicate-key fallback boundaries; object-key-only recursion can miss credentials while line-greedy raw replacements can erase later JSON fields.
- Oversized-payload indicator scans must recognize escaped JSON keys (for example `\"apiKey\":`), sensitive labels used as `name`/`key` values, and all sensitive header tuples (Authorization, Cookie, Set-Cookie, proxy variants), plus singular and plural credential keys before deciding that a payload is safe to persist unchanged.
- Raw payload redaction needs the same normalized sensitive-key classifier for camel-case assignments and prefixed option flags (`proxyAuthorization=...`, `--proxy-authorization ...`, `--openai-api-key ...`) as structured JSON, including the oversized fast path; scheme-bearing authorization options/assignments require redacting both scheme and credential, including every SigV4 parameter, in inert post-tool output.
- Duplicate-key JSON can make structured sanitization report no change when a benign winning property shadows an earlier credential tuple or `{name|key, value}` pair; run a raw structured-secret indicator after parsed traversal. Whole-payload redaction is safe only for inert post-tool output—pre-tool governance must selectively redact/drop the shadowed structure while preserving policy-relevant command fields.
