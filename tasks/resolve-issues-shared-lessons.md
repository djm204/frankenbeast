...
- Treat cron-install taint as a language-aware dataflow problem: cover post-processed/backquoted `printenv`, wrapped or argument-aliased `gh auth token`, aliased env containers/imports/sinks, dot/bracket/destructuring property flows, multiline schedules/assembly/programmatic sinks, and redirect/tee/stdin staging while excluding quoted heredoc and escaped runtime shell variables.
