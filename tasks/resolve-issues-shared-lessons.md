...
- Duplicate-key JSON can make structured sanitization report no change when a benign winning property shadows an earlier credential tuple or `{name|key, value}` pair; run a raw structured-secret indicator after parsed traversal and whole-redact when it finds one.
