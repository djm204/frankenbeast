...
- For state snapshot diff follow-ups, treat direct approval primitive maps (`approvals.json` with token keys) as keyed approval records, prefer card/task identity over mutable worker ownership, skip aggregate wrapper metadata once nested collections are extracted, keep explicit subsystem directories authoritative over generic basenames (`memory/state.json` stays memory), and pass success/error paths through shared redaction so path strings with `token=`/`password=` fragments are scrubbed too.
