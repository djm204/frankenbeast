...
## 2026-07-15 — Runtime config integrity review hardening
- If a runtime config file is integrity-checked and then parsed, verify the exact bytes that will be parsed; separate verify/read calls can leave a same-UID TOCTOU gap even when the digest/HMAC logic is correct.
- Do not put high-value integrity signing keys in allowlisted Docker `-e KEY=value` args or persisted attempt metadata; pass them through a non-persisted env map and assert tests cover absence of the secret value from command args.
- After a CLI consumes a runtime-config signing key, scrub that env var before spawning provider CLIs so downstream tools cannot forge later config/manifest pairs.
