...
- Treat cron-install taint as a language-aware dataflow problem: cover post-processed `printenv`, env-prefixed `gh auth token`, `+=`, optional-chained object properties, equivalent JS/Python env accessors, dynamic Python crontab schedules, and ordinary redirected staging writes while excluding single-quoted or escaped runtime shell variables.
