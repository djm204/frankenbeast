...
- Separate test lanes with directory-level command includes (`test` and `test:unit` on `tests/unit`), keep bare Vitest config/scripts excluding `tests/integration`, and run `test:integration` through a dedicated integration Vitest config; this avoids brittle glob assumptions and ensures a future integration file in `tests/integration/` is never picked up by default test runs even if naming drifts.
...
- When narrowing a package's default test lane, add that package's integration lane to CI/release validation (for example `npm run test:integration --workspace=@franken/governor`) so previously covered integration tests do not silently drop out of release gates without pulling unrelated flaky package integration suites into the same PR.
