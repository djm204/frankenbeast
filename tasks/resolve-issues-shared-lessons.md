...
- Raw payload redaction needs the same normalized sensitive-key classifier for camel-case assignments and prefixed option flags (`proxyAuthorization=...`, `--proxy-authorization ...`, `--openai-api-key ...`) as structured JSON; scheme-bearing authorization options/assignments require redacting both scheme and credential in inert post-tool output.
