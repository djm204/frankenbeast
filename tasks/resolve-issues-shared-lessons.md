...
- Treat cron-install taint as a language-aware dataflow problem: cover post-processed/backquoted `printenv`, env-prefixed or argument-aliased `gh auth token`, `+=`, optional-chain/destructuring property flows, equivalent JS/Python env accessors, multiline schedules/assembly/programmatic sinks, and redirected/stdin staging while excluding quoted heredoc and escaped runtime shell variables.
