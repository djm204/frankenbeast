...
function expectBuildTypecheckLintStep(
  workflow: Record<string, unknown>,
): Record<string, unknown> {
  const jobs = expectRecord(workflow.jobs, "workflow.jobs");
  const buildTestLint = expectRecord(
  ...
  expect(
    step,
    "CI should explicitly gate build, typecheck, and lint through Turbo",
  ).toBeTruthy();
...
`),
    ...
    it("uses the repository-pinned minimum supported Node.js version", () => {
      ...
      const setupNode = expectSteps(buildTestLint).find(
        (step) => step.uses === "actions/setup-node@v4",
      );
      expect(setupNode).toBeTruthy();
      const setupNodeWith = expectRecord(
        setupNode?.with,
    ...
    it("runs npm ci for deterministic installs", () => {
      ...
      expect(
        expectSteps(buildTestLint).some((step) => step.run === "npm ci"),
      ).toBe(true);
    });

    it("runs the guarded security audit after deterministic installs", () => {
      expect(content).toContain("npm run audit:security");
      expect(content.indexOf("npm ci")).toBeLessThan(
        content.indexOf("npm run audit:security"),
      );
    });
    ...
    it("enables Corepack and verifies the packageManager-pinned npm before installing", () => {
      ...
      expect(content.indexOf("npm install -g corepack@0.34.4")).toBeLessThan(
        content.indexOf("corepack enable npm"),
      );
      expect(
        content.indexOf("node scripts/check-package-manager.mjs"),
      ).toBeLessThan(content.indexOf("npm ci"));
    });
    ...
    it("explicitly gates build, typecheck, and lint before running the shared CI test target", () => {
      ...
      expect(buildTypecheckLintStep.name).toBe(
        "Run package build, typecheck, and lint",
      );
      expect(content).toMatch(
        /turbo run build typecheck lint[\s\S]*npm run test:ci/,
      );
      expect(content.indexOf("turbo run build typecheck lint")).toBeLessThan(
        content.indexOf("npm run test:ci"),
      );
    ...
    it("runs the deterministic root Vitest suite through the shared CI test script", () => {
      ...
      expect(
        packageJson.scripts?.["test:ci"]?.indexOf("npm run ci:test:root"),
      ).toBeLessThan(
        packageJson.scripts?.["test:ci"]?.indexOf("npm run ci:test:packages") ??
          -1,
      );
    });
    ...
    it("runs a bootstrap dry-run after deterministic install and fails the workflow on prerequisite errors", () => {
      ...
      expect(content.indexOf("npm ci")).toBeLessThan(
        content.indexOf("npm run bootstrap:dry-run"),
      );
      expect(content.indexOf("npm run bootstrap:dry-run")).toBeLessThan(
        content.indexOf("npm run audit:dependencies"),
      );
    ...
    it("runs CI test commands with a fixed deterministic seed matrix", () => {
      ...
      expect(content.indexOf("FRANKENBEAST_SEED")).toBeLessThan(
        content.indexOf("npm run test:ci"),
      );
    });
    ...
    it("runs test commands through a configurable retry wrapper", () => {
      ...
      expect(packageJson.scripts?.["ci:test:root"]).toBe(
        "node scripts/retry-ci-command.mjs -- npm run test:root",
      );
      expect(packageJson.scripts?.["ci:test:packages"]).toBe(
        "node scripts/retry-ci-command.mjs -- npx turbo run test",
      );
    ...
    it("uses the pinned minimum Node.js version in every CI setup-node step", () => {
      ...
      for (const [jobName, jobConfig] of Object.entries(jobs)) {
        ...
        for (const step of expectSteps(job).filter(
          (candidate) => candidate.uses === "actions/setup-node@v4",
        )) {
          const setupNodeWith = expectRecord(
            step.with,
            `${jobName}.actions/setup-node.with`,
          );
          expect(setupNodeWith["node-version-file"]).toBe(".nvmrc");
          expect(setupNodeWith["node-version"]).toBeUndefined();
        }
    ...
    it("uses actions/checkout with full history for root verification tests", () => {
      ...
      const checkout = expectSteps(buildTestLint).find(
        (step) => step.uses === "actions/checkout@v4",
      );
      expect(checkout).toBeTruthy();
      const checkoutWith = expectRecord(
        checkout?.with,
...
describe("release-please.yml publishes released npm packages", () => {
  ...
  it("validates build, typecheck, test, and lint before release records are created", () => {
    ...
    expect(content).toMatch(
      /Validate release before creating tags[\s\S]*turbo run build typecheck lint[\s\S]*turbo run test/,
    );
    expect(content.indexOf("turbo run build typecheck lint")).toBeLessThan(
      content.indexOf("turbo run test"),
    );
  ...
  it("enforces the packageManager-pinned npm before release installs and publishes", () => {
    ...
    expect(
      content.match(/node scripts\/check-package-manager\.mjs/g)?.length,
    ).toBe(2);
    expect(
      content.indexOf("node scripts/check-package-manager.mjs"),
    ).toBeLessThan(content.indexOf("npm ci"));
    expect(
      content.lastIndexOf("node scripts/check-package-manager.mjs"),
    ).toBeLessThan(content.lastIndexOf("npm ci"));
  });
  ...
  it("defers npm token enforcement until a public package needs publishing", () => {
    const content = readFileSync(RELEASE_PATH, "utf-8");
    const tokenCheckIndex = content.indexOf(
      "NPM_TOKEN secret is required to publish $name@$version",
    );
    const privateSkipIndex = content.indexOf(
      "Skipping $package_path: package is private",
    );
