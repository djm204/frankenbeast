...
Sibling Frankenbeast packages such as `@franken/governor`, `@franken/brain`, and `@franken/types` use TypeScript strict mode. Consistency across the monorepo is essential for cross-module type safety.

## Decision

Use **TypeScript (strict mode)** targeting Node.js `>=22.13.0 <23 || >=24.0.0 <26`, compiled with `tsc`.

- `tsconfig.json`: `strict: true`, `noUncheckedIndexedAccess: true`, `exactOptionalPropertyTypes: true`
- ESM modules (`"type": "module"` in `package.json`)
- `module: "NodeNext"`, `moduleResolution: "NodeNext"` (Pattern A, consistent with `@franken/governor`, `@franken/brain`, and `@franken/types`)
