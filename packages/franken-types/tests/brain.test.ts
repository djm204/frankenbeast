...
import {
  ...
  type IBrain,
  type IActionFaculty,
  type ILearningFaculty,
  type IPlanningFaculty,
  type IReasoningFaculty,
  type IWorkingMemory,
...
describe('Brain interfaces (type-level)', () => {
  it('IBrain has required shape', () => {
    const brain: IBrain = {
      ...
      planning: { kind: 'planning', configured: false } as IPlanningFaculty,
      reasoning: { kind: 'reasoning', configured: false } as IReasoningFaculty,
      action: { kind: 'action', configured: false } as IActionFaculty,
      learning: { kind: 'learning', configured: false } as ILearningFaculty,
      rightToForget: () => ({
        selectorHash: 'hash',
        dryRun: false,
