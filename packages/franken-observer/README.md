...
When `send(payload, { idempotencyKey, target })` receives an idempotency key, `WebhookNotifier` records a delivery receipt with `sent`, `skipped`, or `failed`, an ISO timestamp, the logical target, and a SHA-256 content hash. A later call with the same key, target, and content hash is skipped after a successful send; changed content, a different target, or a prior failed receipt is delivered again. The in-memory receipt store is useful for one process, while status, approval, and doctor/loop notification runners should pass a durable `WebhookDeliveryReceiptStore` so repeated cron/status runs do not duplicate Discord or webhook messages across restarts.
