...
For Beast controls, set `FRANKENBEAST_BEAST_OPERATOR_TOKEN` in the repo root `.env` to the token generated by `frankenbeast init`; the Vite dev proxy reads it server-side without exposing it to the browser.
