...
describe('GovernorAdapter', () => {
  ...
  it('redacts memory retention report context before shared governor logging', async () => {
    ...
    await expect(governor.check({
      ...
      context: '{"readScope":"agent","agentId":"alice@example.test","now":"alice@example.test invalid date","maxEntries":10,"maxScanRows":25,"legacy":"secret"}',
    })).resolves.toMatchObject({ decision: 'approved' });

    await expect(governor.check({
      action: 'execute_tool',
      context: '{"tool":"fbeast_memory_retention_report","args":{"readScope":"agent","agentId":"bob@example.test","expiryHorizonMs":1000,"maxScanRows":50,"legacy":"secret"}}',
    })).resolves.toMatchObject({ decision: 'approved' });
    ...
    const rows = db.prepare(`SELECT context FROM governor_log ORDER BY id ASC`).all() as Array<{ context: string }>;
    db.close();
    expect(rows[0]!.context).toBe('{"readScope":"agent","now":"[memory-retention-report-args-redacted]","maxEntries":10,"maxScanRows":25,"agentId":"[memory-retention-report-args-redacted]"}');
    expect(rows[1]!.context).toBe('{"tool":"fbeast_memory_retention_report","args":{"readScope":"agent","expiryHorizonMs":1000,"maxScanRows":50,"agentId":"[memory-retention-report-args-redacted]"}}');
