...
TOOL_CONTEXT=$(printf '%s' "$INPUT" | "$NODE_BIN" -e "const fs = require('node:fs'); try { const d = JSON.parse(fs.readFileSync(0, 'utf8')); const ti = d?.tool_input; const tn = d?.tool_name || ''; const keys = ['command', 'cmd', 'commands', 'args', 'argv', 'script']; const memoryEvidenceKeys = ['key', 'category', 'sourceScope', 'query', 'dryRun', 'profile', 'activeProfile', 'crossProfile', 'redaction']; let out = ''; if (typeof ti === 'string') { out = ti; } else if (ti && typeof ti === 'object' && !Array.isArray(ti) && tn !== 'apply_patch') { if (tn === 'fbeast_memory_right_to_forget' || tn === 'fbeast_memory_forget' || tn === 'fbeast_memory_store' || tn === 'fbeast_memory_export') { const clone = {}; for (const key of memoryEvidenceKeys) { if (!Object.prototype.hasOwnProperty.call(ti, key)) continue; clone[key] = ['key', 'category', 'sourceScope', 'query'].includes(key) ? '[right-to-forget-selector-redacted]' : ti[key]; } out = JSON.stringify(clone); } else { const parts = []; for (const key of keys) { const value = ti[key]; if (value === undefined) continue; if (Array.isArray(value)) { parts.push(value.map(String).join(' ')); } else if (typeof value === 'string') { parts.push(value); } else { parts.push(JSON.stringify(value)); } } out = parts.join(' '); } } process.stdout.write(tn === 'apply_patch' ? '' : out); } catch { process.stdout.write(''); }" 2>/dev/null || echo "")
