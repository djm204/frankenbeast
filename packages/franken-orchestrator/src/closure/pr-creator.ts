...
const GH_AUTH_FAILURE_RE = /gh auth login|not (?:logged in|authenticated)|authentication (?:required|failed)|requires authentication|bad credentials|http 401|http 403|resource not accessible by integration|to get started with github cli|set the GH_TOKEN environment variable|GH_TOKEN/i;
...
function buildGhAuthRequiredAction(branch: string, details: unknown): PrCreationRequiredActionError {
  ...
  const isIntegrationPermissionFailure = /resource not accessible by integration|http 403/i.test(detailText);
  const action = isActionsTokenFailure || isIntegrationPermissionFailure
  ...
  const message = isActionsTokenFailure || isIntegrationPermissionFailure
