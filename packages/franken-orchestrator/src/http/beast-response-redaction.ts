...
const EMBEDDED_HOST_PATH_RE = /(^|[\s\p{P}\p{S}])(\/(?:home|Users|private|var|tmp|srv|opt|etc|root|mnt|workspace|workspaces)\/(?:[^\s"']+\/?)+|[A-Za-z]:[\\/](?:[^\s"']+)|\\\\(?:[^\s"']+))/gu;
const EMBEDDED_POSIX_PATH_RE = /(^|[\s\p{P}\p{S}])(\/(?:[^/\s"']+\/)+[^\s"']+)/gu;
