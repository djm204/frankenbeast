...
describe('ProcessBeastExecutor', () => {
  ...
  it('stops the current attempt without deleting the run row', async () => {
    ...
    expect(supervisor.stop).toHaveBeenCalledWith(777, { processGroupOwned: true });
    expect(repo.getRun(run.id)).toMatchObject({
      status: 'stopped',
      currentAttemptId: attempt.id,
      attemptCount: 1,
  ...
  describe('handleProcessExit', () => {
    ...
    it('redacts secrets from failed attempt stderr tails in repository events and eventBus publishes', async () => {
      ...
      cb.onStderr(JSON.stringify({ password: jsonPassword, client_secret: jsonSecret, botToken: camelToken }));
      cb.onStderr(JSON.stringify({ password: escapedPassword, accessToken: camelAccessToken }));
      cb.onStderr(`redis://:${cachePassword}@localhost:6379/0`);
      const basicAuthValue = ['Basic', basicToken].join(' ');
      ...
      expect(serializedPersistedEvent).not.toContain(jsonPassword);
      expect(serializedPersistedEvent).not.toContain(jsonSecret);
      expect(serializedPersistedEvent).not.toContain(camelToken);
      expect(serializedPersistedEvent).not.toContain(escapedPassword);
      expect(serializedPersistedEvent).not.toContain(camelAccessToken);
      ...
      expect(serializedPublishedEvent).not.toContain(jsonPassword);
      expect(serializedPublishedEvent).not.toContain(jsonSecret);
      expect(serializedPublishedEvent).not.toContain(camelToken);
      expect(serializedPublishedEvent).not.toContain(escapedPassword);
      expect(serializedPublishedEvent).not.toContain(camelAccessToken);
