...
describe('provider CLI availability preflight', () => {
  ...
  it('preflights the plan-build provider override for planning runs', () => {
    expect(resolveEffectivePreflightProvider('claude', {
      provider: 'claude',
      llmConfig: {
        default: { provider: 'codex' },
  ...
  it('does not preflight plan-build providers for interview-only runs', () => {
    expect(resolveEffectivePreflightProviders('claude', {
      provider: 'claude',
      llmConfig: {
        default: { provider: 'codex' },
    ...
    }, 'interview', 'interview')).toEqual(['gemini']);
  });
...
describe('main() execution', () => {
  ...
  it('refuses to start chat-server while detected daemon is draining', async () => {
    ...
    await expect(main()).rejects.toThrow('Detected a draining beasts-daemon pid file');
