...
describe('RunConfigLoader', () => {
  ...
  describe('loadRunConfig', () => {
    ...
    it('fails closed for malformed integrity manifests without parsing the config', async () => {
      workDir = await mkdtemp(join(tmpdir(), 'run-config-'));
      const filePath = join(workDir, 'malformed-manifest.json');
      await writeFile(filePath, JSON.stringify({ provider: 'claude' }));
      const manifestPath = join(workDir, 'malformed-manifest.json.integrity');
    ...
    it('fails closed when a signed manifest is replayed for a different config path', async () => {
      workDir = await mkdtemp(join(tmpdir(), 'run-config-'));
      const filePath = join(workDir, 'config.json');
      const otherPath = join(workDir, 'other-config.json');
      const bytes = JSON.stringify({ provider: 'claude' });
      await writeFile(filePath, bytes);
      const manifestPath = join(workDir, 'other-config.json.integrity');
      await writeFile(manifestPath, JSON.stringify(createRunConfigIntegrityManifest(bytes, 'trusted-secret', {
        configPath: otherPath,
      })));
      process.env[RUN_CONFIG_INTEGRITY_ENV] = manifestPath;
      process.env[RUN_CONFIG_INTEGRITY_SECRET_ENV] = 'trusted-secret';

      expect(() => loadRunConfig(filePath)).toThrow('runtime config manifest path mismatch');
    });
