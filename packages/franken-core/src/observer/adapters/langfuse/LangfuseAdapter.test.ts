...
describe('LangfuseAdapter', () => {
  ...
  describe('flush()', () => {
    it('POSTs to the Langfuse OTEL endpoint', async () => {
      const adapter = new LangfuseAdapter({ publicKey: 'pk-test', secretKey: 'sk-test', fetch: mockFetch as any })
    ...
    it('sends a Basic Authorization header using publicKey:secretKey', async () => {
      const adapter = new LangfuseAdapter({ publicKey: 'pk-abc', secretKey: 'sk-xyz', fetch: mockFetch as any })
      await adapter.flush(makeTrace())
    ...
    it('throws if the HTTP response is not ok', async () => {
      ...
      const adapter = new LangfuseAdapter({ publicKey: 'bad', secretKey: 'bad', fetch: mockFetch as any })
