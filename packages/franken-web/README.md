...
## Safe rendering rules for untrusted text

Issue bodies, PR comments, transcript messages, runtime events, logs, analytics details, and future memory excerpts are untrusted operator-facing text. The dashboard renders these values through the `SafeMarkdownText` plain-text boundary instead of parsing markdown or HTML.

Allowed behavior:

- escaped text nodes only; raw HTML is never inserted into the DOM
- existing surface styles preserve line breaks, indentation, and readable wrapping
- JSON/log detail panes keep their original textual content for copying and audit

Blocked behavior:

- `<script>`, `<iframe>`, SVG/HTML tags, malformed HTML, and inline event handlers such as `onerror` are displayed as text, not elements or attributes
- markdown links, images, `javascript:` URLs, and `data:` URLs from untrusted text are not materialized as clickable DOM links
- no `dangerouslySetInnerHTML` is used for untrusted dashboard content
