...
- **Root test entrypoints** — `npm test` runs the default deterministic package suites, `npm run test:root` runs root-only Vitest tests, and `npm run test:integration` runs every workspace integration suite exposed through Turborepo.
