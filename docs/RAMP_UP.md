...
3. **Init-only `--non-interactive` support**: `frankenbeast init --non-interactive` disables guided prompts for setup and can be paired with `--backend`, `--verify`, or `--repair`. Broader headless execution still starts at `plan` or `run` with existing inputs.
