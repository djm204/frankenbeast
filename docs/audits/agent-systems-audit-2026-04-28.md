...
### 2026-05-23 Chunk 4 Follow-up

- **Checkpointing is partial, not full state-machine persistence** → `partially-fixed`. Beast phase snapshots are now appended to `.fbeast/state/<runId>.jsonl`, and LLM/tool payloads are available through content-addressed replay blobs. Full process checkpoint/restore, filesystem state, and OS-level execution replay remain out of scope. Evidence: `packages/franken-orchestrator/src/context/state-snapshot-store.ts`, `packages/franken-orchestrator/src/beast-loop.ts`, `packages/franken-observer/src/replay/replay-content-store.ts`; tests in `packages/franken-orchestrator/tests/unit/beast-loop-state-persistence.test.ts` and `packages/franken-observer/src/replay/replay-content-store.test.ts`.
- **The main Beast loop is phased but not modeled as a persisted finite-state machine** → `fixed` for the Beast-loop phase FSM. Phase transitions are persisted after ingestion, hydration, planning, execution, and closure. Evidence: `packages/franken-orchestrator/src/beast-loop.ts`, `packages/franken-orchestrator/src/context/state-snapshot-store.ts`; test: `packages/franken-orchestrator/tests/unit/beast-loop-state-persistence.test.ts`.
- **Replay is timeline analysis, not deterministic execution replay** → `fixed` at the record-replay level. `DeterministicReplayer` replays saved LLM responses and tool results from sha256-verified content-addressed blobs. OS process/syscall/tool side-effect replay remains out of scope. Evidence: `packages/franken-observer/src/replay/deterministic-replayer.ts`, `packages/franken-observer/src/replay/replay-content-store.ts`; tests in `packages/franken-observer/src/replay/deterministic-replayer.test.ts`.
- **LLM prompts and responses are not universally persisted into the observer audit trail** → `partially-fixed`. CLI LLM requests/responses and CLI skill tool calls/results are captured through the CLI adapter/executor paths and persisted as replay manifests/blobs. Future execution paths that bypass those adapters still need explicit replay capture. Evidence: `packages/franken-orchestrator/src/adapters/cli-llm-adapter.ts`, `packages/franken-orchestrator/src/skills/cli-skill-executor.ts`, `packages/franken-orchestrator/src/adapters/audit-observer-adapter.ts`, `packages/franken-orchestrator/src/adapters/cli-observer-bridge.ts`; tests in `packages/franken-orchestrator/tests/unit/adapters/cli-llm-adapter.test.ts`, `packages/franken-orchestrator/tests/unit/skills/cli-skill-executor.test.ts`, `packages/franken-orchestrator/tests/unit/adapters/audit-observer-adapter.test.ts`, and `packages/franken-orchestrator/tests/unit/adapters/cli-observer-bridge.test.ts`.
- ADR: `docs/adr/037-durable-audit-and-deterministic-replay.md` records the durable-audit/replay boundary and residual claims.
