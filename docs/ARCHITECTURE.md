...
| Package | Role |
|---------|------|
| `@franken/brain` | SQLite-backed memory plus a process-local `BrainRegistry` and additive planning/reasoning/action/learning faculty surfaces. Consolidated Beast dependencies attach planning to the existing planner and episodic lifecycle recording, and reasoning to the critique chain; action/learning adapters and durable agent-type paths remain follow-up work. |
