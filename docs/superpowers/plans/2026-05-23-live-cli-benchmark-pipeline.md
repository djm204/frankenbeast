...
If exact CLI config paths are uncertain, implement the file plan and leave one `live-client-smoke follow-up` comment for the later live smoke task; tests should verify the plan shape rather than pretending to know unverified CLI internals.
