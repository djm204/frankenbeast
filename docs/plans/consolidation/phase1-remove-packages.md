...
6. For each import found: remove entirely, or replace with a temporary pass-through that preserves the interface contract with a `// Phase N replacement placeholder` comment. Pass-throughs must satisfy the type signature and may delegate to a no-op, but must be tracked and replaced by the phase indicated — no stubs ship as final product.
