...
- **Chat provider self-awareness:** `CliLlmAdapter` reports `providerContext` (`provider`, `model`, and `switchedFrom`/`switchReason` when a rate-limit/unavailability fallback actually occurred) alongside usage. `ConversationEngine` injects this as a "Runtime status: …" prompt note using the *last known* state (`priorProviderContext`) — this turn's own fallback isn't knowable until after it completes, so it surfaces starting next turn. `ChatRuntimeState.lastProviderContext` is carried forward by the caller like `pendingApproval`; do not add it as internal mutable state on the shared `ChatRuntime`/`ConversationEngine` instances (both are shared across concurrent HTTP/WS sessions).
