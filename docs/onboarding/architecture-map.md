...
| `packages/franken-mcp-suite/` (`@franken/mcp-suite`) | `mcp-suite` | `mcp-maintainers` / `core-maintainers` | `fbeast` CLI, MCP server/proxy registration, hooks, tool adapters, governance gate integration, and local client setup. | `src/cli/`, `src/servers/`, `src/shared/`, hook/setup tests. | `npm run build --workspace @franken/mcp-suite && npm run test --workspace @franken/mcp-suite` |
...
| Onboarding, architecture, runbook, or docs-only issue | `docs/onboarding/` or the nearest docs area | Root `README.md` and `ONBOARDING.md` entrypoints so new agents find the doc. | Focused root docs test for the touched file/issue; do not reuse `tests/docs-issue-1666.test.ts` for unrelated docs unless that map or its entrypoints changed. |
