...
| Usage-limited | A review gate, provider route, or worker attempt stopped because the provider or bot reported quota or rate-limit exhaustion. | The latest provider or bot message, reset/retry window, trigger count, and whether a monitor owns the retry. | Do not treat it as clean or failed work; park the worker or request an explicit retry/bypass decision. |
| Stale-head clean | A clean review, CI result, or summary that applies to an older PR commit instead of the current head. | Current PR head SHA, reviewed SHA named by the bot/check, and whether new commits landed after the clean signal. | Re-trigger or re-run the gate for the current head before merging or reporting readiness. |
| Unresolved review threads | Review comments, usually from Codex, that remain open in GitHub's review-thread state even if later comments appear clean. | GraphQL review-thread status, thread author, latest reply, and whether the finding still applies to the current head. | Reply with the fix or rationale, resolve the thread, and verify the unresolved count is zero before merging. |
...
| Approval token state | The durable status of an approval runner request, such as requested, pending, approved, executed, denied, stale, or failed. | Token id, command/effect summary, worktree, approver/status, execution result, and whether the target PR/card has moved on. | Execute only approved current tokens through the runner; clear stale terminal tokens with evidence rather than replaying them. |
