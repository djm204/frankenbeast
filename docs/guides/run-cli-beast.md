...
The default env allowlist is intentionally narrow: `PATH`, `HOME`, `LANG`, `LC_ALL`, `FRANKENBEAST_RUN_CONFIG`, `FRANKENBEAST_RUN_CONFIG_INTEGRITY`, `FRANKENBEAST_RUN_CONFIG_INTEGRITY_SECRET`, `FRANKENBEAST_SPAWNED`, and the `FRANKENBEAST_MODULE_*` toggles for firewall, skills, memory, planner, critique, governor, and heartbeat. The daemon signs each per-run config snapshot before spawning the CLI; the child verifies that signature before parsing the runtime config and fails closed when the manifest is missing, invalid, stale, tampered, replayed for another path, or signed with an untrusted secret. Manual edits to `.fbeast/.build/run-configs/*.json` are not supported after dispatch; relaunch/re-dispatch to change runtime configuration. Emergency operator-approved bypasses must set `FRANKENBEAST_RUN_CONFIG_INTEGRITY_BYPASS=1`, which emits a clear warning and should not be used as the default path. Secrets such as `GITHUB_TOKEN`, provider API keys, and arbitrary shell environment variables are not inherited unless the Beast definition explicitly places them in `spec.env` and the runtime policy allows the key.
