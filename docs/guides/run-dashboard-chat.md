...
Set `VITE_PROJECT_ID` when you want dashboard chat sessions scoped to a named local project instead of the shared `default` namespace. The frontend passes that value to the chat session API when it lists and resumes sessions, so using a stable per-project value such as `my-project` keeps parallel local checkouts from mixing dashboard chat history. Leave it unset only when the fallback `default` project id is acceptable.
